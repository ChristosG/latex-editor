...
public class MainWindow {
	...
	public void editContents(String type) {
		...
		latexEditorView.setLatexCommand(type);
		latexEditorView.setText(editorPane.getText());
		latexEditorView.setEditorPane(this.editorPane);	
	...
	private void initialize() {
		...
		JMenu mnEncryptStrategy = new JMenu("Encryption");
		menuBar.add(mnEncryptStrategy);
		
		JMenu mntmEncryptedSave = new JMenu("Save Encryption");
		mnEncryptStrategy.add(mntmEncryptedSave);
		
		JMenuItem mntmEncryptedLoad = new JMenuItem("Load Encryption");
		...
		mnEncryptStrategy.add(mntmEncryptedLoad);
			
		JMenuItem rot13 = new JMenuItem("Rot-13");
		rot13.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (option == JFileChooser.APPROVE_OPTION) {
					String filename = filechooser.getSelectedFile().toString();
					if (filename.endsWith(".texR") == false) {
						filename = filename + ".texR";
					}
					latexEditorView.setFilename(filename);
					latexEditorView.setEncryption("rot13");
					latexEditorView.getController().enact("SaveEncrypt");
				}
			}
		});
		mntmEncryptedSave.add(rot13);
		...
		atBash.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (option == JFileChooser.APPROVE_OPTION) {
					String filename = filechooser.getSelectedFile().toString();
					if (filename.endsWith(".texA") == false) {
						filename = filename + ".texA";
					}
					latexEditorView.setFilename(filename);
					latexEditorView.setEncryption("atbash");
					latexEditorView.getController().enact("SaveEncrypt");
